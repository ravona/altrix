...
const TextPlayer: React.FC<Story> = (props) => {
    ...
    const [myTextPlayer, setMyTextPlayer] = useState<TextPlayerClass | null>(
        null,
    );

    useEffect(() => {
        setMyTextPlayer(new TextPlayerClass(props));
    }, []);

    useEffect(() => {
        const currentFrame = myTextPlayer?.getTextFrames()[state.context.index];
        if (currentFrame) {
        ...
        <article className={styles['TextPlayer']}>
            ...
            <div className={styles['TextPlayer__Screen']} ref={containerRef}>
                <div className={styles['TextPlayer__Content']}>
                    {myTextPlayer?.getTextFrames().map((frame, index) => (
                        <p
                            onClick={() => send({ type: 'SET_INDEX', index })}
                            key={frame.id}
                            id={frame.id}
                            className={`${styles['TextPlayer__Chunk']} ${index === state.context.index ? styles['is-active'] : ''}`}
                        >
                            {frame.content}
                        </p>
                    ))}
            ...
            <footer className={styles['TextPlayer__Footer']}>
                <Slider
                    ...
                    max={
                        myTextPlayer?.getTextFrames().length
                            ? myTextPlayer?.getTextFrames().length - 1
                            : 0
                    }
                />
